                    ...
                    new TickerBehaviour(this, 100) {
                        ...
                        protected void onTick() {
                            if (!messageSent) {
                                ...
                                // System.out.println(String.format("[%s] Calling %s for available
                                // slots.", getLocalName(), restaurantName));
                                messageSent = true;
                            }
                        }
                    });
                        ...
                        public void action() {
                            ...
                            if (message != null) {
                                ...
                                if (content.startsWith("SLOTS")) {
                                    Integer slots =
                                            Integer.parseInt(content.replaceFirst("SLOTS_", ""));
                                    // System.out.println(String.format("[%s] Received %d free slots
                                    // from %s.", getLocalName(), slots, sender));
                                    ...
                                    if (slots > 0) {
                                        ...
                                        System.out.println(
                                                String.format(
                                                        "[%s] Making a random reservation at %s.",
                                                        getLocalName(), sender));
                                    } else {
                                        selected = 1 + rand.nextInt(capacities.length - 1);
                                        messageSent = false;
                                ...
                                switch (content) {
                                    case "ADMITTED":
                                        restaurantIdx =
                                                Integer.parseInt(
                                                        sender.replaceFirst("Restaurant_", ""));
                                        break;
                                    case "REJECTED":
                                        selected = 1 + rand.nextInt(capacities.length - 1);
                                        messageSent = false;
