...
public class Species {
    ...
    public static void paintAllSpecies(Graphics g) {
        for (Map.Entry<String, Species> entry : ALL_SPECIES.entrySet()) entry.getValue().paint(g);
    }
    ...
    private void paint(Graphics g) {
        g.setColor(color);
        for (Map.Entry<String, FieldUnity> entry : members.entrySet()) entry.getValue().paint(g);
    }
}
