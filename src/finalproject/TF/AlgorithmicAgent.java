...
/**
 * @author G6: Sebastián Contreras
 */
public class AlgorithmicAgent extends FieldAgent {
    private Timer timer;
                            ...
                            evt -> {
                                ...
                                if (aux_repro == 4) {
                                    ...
                                    for (FieldUnity ally : unidad_local.species.members.values()) {
                                        if (ally.is_alive) cnt++;
                                    }
                                    System.out.println("la cnt es " + cnt);
                                    if (cnt <= 20) status = STATES.REPRODUCING;
                                }

                                switch (status) {
                                    case GOINGTOFIGHT:
                                        if (getLocal().goal_enemy != null
                                                && !getLocal().goal_enemy.is_alive)
