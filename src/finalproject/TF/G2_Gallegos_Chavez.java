...
public class G2_Gallegos_Chavez extends FieldAgent{
    ...
    protected void Reinforce() {
        if(t_Ref == null) {
            t_Ref = new Timer(2000, new ActionListener() {
                @Override
    ...
    protected void Reproduce(){
        //int veces = 0;
        if (t_Rep == null){
            t_Rep = new Timer(4000, new ActionListener() {
                @Override
                public void actionPerformed(ActionEvent evt) { 
    ...
    protected void init(){
        ...
        Reproduce();
        if (veces % 20 == 0){dm = true;}
        else {dm = false;}
        veces = veces + 1;
        if (dm == true){t_Rep.start();}
        else{t_Rep.stop();}
        Reinforce();
        if (dc == true){t_Ref.start();}
        //else{t_Rep.stop();}
    }
