...
/**
 * @author G13: Porta & Morales
 */
public class TilinAgent extends FieldAgent {
    private Timer timer;
    double xdir = 1.0;
    double ydir = 1.0;
        ...
        if (status == STATES.PATROLING) {
            if (msg.getContent().contains(ENEMY + "_")) {
                ...
                if (enemy != null) {
                    ...
                    } else if (getLocal().getPoints() < enemy.getPoints()
                            && getLocal().position.distance(enemy.position) < 20) {
                        // getLocal().setGoal((int)MAX_X/2,(int)MAX_Y/2);
                        // getLocal().goToGoal();
