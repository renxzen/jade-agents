            ...
            parallel.addSubBehaviour(
                    new CyclicBehaviour(this) {
                        ...
                        public void action() {
                            ...
                            if (msg != null) computeMessage(msg);
                            else block();
                        }
                    });
        ...
        if (HostAgent.STARTED) {
            if (getLocal().getPoints() <= 0) {
                ...
                switch (status) {
                    ...
                    case GOINGTOFIGHT:
                        getLocal().goToGoal();
                        if (getLocal().hasArrived()) status = STATES.FIGHTING;
                        break;
                    case GOINGTOSOURCE:
                        getLocal().goToGoal();
                        if (getLocal().hasArrived()) status = STATES.EATING;
                        break;
                    case FIGHTING:
                        fight();
                        break;
                    case EATING:
                        ...
                        // System.out.println(getLocal().getName()+ " " + getLocal().position + " "
                        // + getLocal().getGoal() + " " + source);
                        if (source != null) {
                            // getLocal().fill(source.eat());
                            HostAgent.fill_stock((FieldAgent) this, source.eat());
                            if (source.isEmpty()) status = PATROLING;
                        } else status = PATROLING;
                        break;
                    case ESCAPE:
                        break;
                    case DEAD:
